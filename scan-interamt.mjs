 ...
 *
 * Usage:
 *   node scan-interamt.mjs --user USER
 *   node scan-interamt.mjs --user USER --dry-run
 *   node scan-interamt.mjs --user USER --all            # skip date filter (use for first scan)
 *   node scan-interamt.mjs --user USER --keyword "Softwareentwickler"
 */

import { chromium } from 'playwright';
import { readFileSync, existsSync, mkdirSync } from 'fs';
import yaml from 'js-yaml';
import { appendToPipeline, appendToScanHistory, configureScanUserPaths, loadSeenUrls } from './scan.mjs';
...
const PORTALS_PATH    = userPath(userContext, 'portals.yml');
const SCAN_HISTORY    = userPath(userContext, 'data/scan-history.tsv');
const DEBUG_SCREENSHOT = userPath(userContext, 'output/debug-interamt.png');
const DEBUG_HTML = userPath(userContext, 'output/debug-interamt.html');
...
async function searchInteramt(page, keyword, isFirst) {
  ...
  if (DEBUG) {
    ...
    mkdirSync(userPath(userContext, 'output'), { recursive: true });
    await page.screenshot({ path: DEBUG_SCREENSHOT, fullPage: true });
    const { writeFileSync: wf } = await import('fs');
    wf(DEBUG_HTML, await page.content());
...
async function main() {
  mkdirSync(userPath(userContext, 'data'), { recursive: true });
  ...
  if (newOffers.length > 0) {
    ...
    if (DRY_RUN) {
      ...
      console.log(`\nSaved to users/${userContext.userId}/data/pipeline.md`);
    }
  }
