...
- `resolve_duplicate` for a confirmed duplicate tracker/report group. `resolve-verify-warnings.mjs` validates the exact candidate partition, preserves the most advanced lifecycle state (`Applied > Rejected > Evaluated`), backs up state, archives losing artifacts, and records its ledger. When an `Applied`-or-later duplicate owns the report and generated CV that were actually used, those report/HTML/PDF contents are promoted and renamed into the canonical keeper ID instead of being discarded; the superseded keeper artifacts are archived.
