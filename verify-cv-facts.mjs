...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (rawArgs.includes('--self-test')) {
    ...
    const sourceArgs = [];
    let targetArg = '';
    let configPath = userContext.userRoot
    ...
    if (!targetArg || args.includes('--help') || args.includes('-h')) {
      console.log(`Usage: node verify-cv-facts.mjs --user <id> <generated-cv> [--source path] [--config path]
...
Default config:  users/<id>/config/cv-facts.json (optional)`);
    ...
    const sources = sourceArgs.length > 0 ? sourceArgs : defaultSources;
