...
function makeLane() {
  ...
  const userRoot = join(dir, 'users', 'test');
  mkdirSync(join(userRoot, 'data'), { recursive: true });
  writeFileSync(join(userRoot, 'data', 'applications.md'), TRACKER);
  const portals = join(userRoot, 'portals.yml');
  writeFileSync(portals, PORTALS);
  return { dir, userRoot, portals };
}
...
const runScan = (dir, env) => execFileSync(NODE, [join(ROOT, 'scan.mjs')], {
  ...
  env: {
    ...
    CAREER_OPS_USERS_DIR: join(dir, 'users'),
    ...env,
  },
  encoding: 'utf-8',
...
{
  const { dir, userRoot, portals } = makeLane();
  try {
    runScan(dir, { CAREER_OPS_PORTALS: portals });
    const defaultEntries = entries(join(userRoot, 'data', 'pipeline.md')).length;
    const defaultHistory = existsSync(join(userRoot, 'data', 'scan-history.tsv'));
...
//    pass just as happily if scan wrote to BOTH.
{
  ...
  try {
    ...
    const defaultTouched = existsSync(join(userRoot, 'data', 'pipeline.md'))
      || existsSync(join(userRoot, 'data', 'scan-history.tsv'));
