...
try {
  ...
  if (
    ...
    pass('CLAUDE.md delegates onboarding state to AGENTS.md and doctor --json with autoCopied');
  } else {
    fail('Agent docs still duplicate onboarding prerequisite checks or omit user-scoped doctor --json/autoCopied');
  }
} catch (e) {
...
try {
  ...
  const out = run('bash', [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1', '--max-retries', '3', '--rate-limit-sleep', '0'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
  ...
  const dry = run('bash', [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--resume-paused', '--dry-run'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
  ...
  const statusOnly = run('bash', [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--status'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
