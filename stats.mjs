...
let userContext;
try {
  userContext = getUserContext(process.argv.slice(2), { requireUser: IS_MAIN });
} catch (err) {
  printUserContextErrorAndExit(err);
}
const APPS_FILE = userContext.userRoot ? userPath(userContext, 'data/applications.md') : '';
const SCAN_HISTORY_FILE = userContext.userRoot ? userPath(userContext, 'data/scan-history.tsv') : '';
...
// Hired is the fullest pursuit of all, so it belongs here.
