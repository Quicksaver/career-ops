...
- Web report decision view: `/pipeline/{id}` uses a `96rem` maximum width and promotes non-empty `hard_stops`, `soft_gaps`, and `top_strengths` from the report's fenced YAML Machine Summary into a dedicated table immediately above the Verdict callout. Parsing is tolerant and read-only; malformed or legacy summaries keep the existing report rendering, and the raw Machine Summary remains in Technical details.
