 ...
 *
 * Run: node weekly-digest.mjs --user USER                       (JSON to stdout)
 *      node weekly-digest.mjs --user USER --summary              (human-readable digest)
 *      node weekly-digest.mjs --user USER --from 2026-07-13 --to 2026-07-19
...
export function loadSessions(dir) {
  if (!dir) throw new Error('sessionsDir is required');
...
export function computeWeeklyDigest({
  ...
  to,
  sessionsDir,
  questionBankPath = sessionsDir ? join(dirname(sessionsDir), 'question-bank.md') : null,
} = {}) {
  if (!sessionsDir) throw new Error('sessionsDir is required');
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  let userContext;
  try {
    userContext = getUserContext(rawArgs, { requireUser: flagValue(rawArgs, '--dir') === undefined });
  } catch (err) {
  ...
  const sessionsDir = dirValue ? dirValue : userPath(userContext, 'interview-prep/sessions');
  const questionBankPath = dirValue
    ? join(dirname(sessionsDir), 'question-bank.md')
    : userPath(userContext, 'interview-prep/question-bank.md');
  ...
  let result;
  try {
    result = computeWeeklyDigest({ from, to, sessionsDir, questionBankPath });
  } catch (err) {
