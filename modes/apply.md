...
> Apply `users/{USER}/voice-dna.md` (if present) to free-text answers and cover-letter fields — full guardrail, conversational voice included (Tier 1 + Tier 2). See `_writing.md` → Voice DNA.
...
3. SEARCH      → Match against existing reports in users/{USER}/reports/
...
2. Search in `users/{USER}/reports/` by company name (case-insensitive grep)
...
- **If re-evaluate**: Execute full A-F evaluation, update report, regenerate Section H / Application Answers
- **Update tracker**: Change role title in `users/{USER}/data/applications.md` if applicable
...
- **New question** → generate response from the report + `users/{USER}/cv.md`
...
```bash
node application-answers.mjs --user {USER} --report users/{USER}/reports/NNN-company-role-date.md --input answers.json --state filled
```

## Step 9 — Post-apply (optional)

If the candidate confirms that they submitted the application:
1. Update status to Applied via the canonical CLI: `node set-status.mjs --user {USER} <report#> Applied` (never hand-edit the table). If the candidate submitted on a different day than today, add `--on YYYY-MM-DD` with the actual submission date — the active user's status ledger should record when it happened, not when it was typed in.
2. Seed the follow-up schedule: run `node followup-seed.mjs --user {USER} {num} --json` (where `{num}` is the tracker row number). If the candidate applied on a different day than today, pass `--date YYYY-MM-DD` with the actual submission date. It is idempotent, so re-running is safe. (`--on` and `--date` are the same concept — the real submission date — each under its own script's flag name; pass the same value to both.)
