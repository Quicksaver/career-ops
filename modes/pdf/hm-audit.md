...
An opt-in pass inside `modes/pdf.md`, run at Step 20 — between the fact gate and the PDF render — when the invocation carried `--hm-audit` or `users/{USER}/modes/_custom.md` turns it on. Not a mode of its own: `pdf` has already loaded the system shared mode plus the active user's `_profile.md` and `_custom.md` by the time this runs, and those rules govern what the audit may recommend.
...
The fact gate at `pdf` Step 19 (`verify-cv-facts.mjs`) is **mechanical**: it diffs generated output against `users/{USER}/cv.md` and `users/{USER}/article-digest.md` to catch invented metrics. It cannot judge whether a truthful bullet is the *right* bullet — buried lede, wrong altitude, wrong vocabulary, or answering a requirement the JD never raised. This pass answers a different question: *"Would the person who screens this actually advance it?"*
...
**Never audit `users/{USER}/cv.md` itself.** That is the untailored master; auditing it produces confidently wrong verdicts about a CV the user never intends to send.

## Inputs

1. **Role** — a report number or company slug. If omitted, use the most recent evaluated role (same argument pattern as `cover`).
2. **Report** at `users/{USER}/reports/{num}-{company}-{date}.md` — read for the `**URL:**` header, archetype, and identified gaps.
3. **JD** at `users/{USER}/jds/{slug}.md`, or fetched from the report's `**URL:**`.
4. **Tailored bullets** — resolve the artifact for *this* role, in order. Never parse the `.pdf`.
   1. `/tmp/cv-{REPORT_NUM}-{company-slug}-{YYYY-MM-DD}.json` from the same session — the cleanest source (`experience[].bullets[]`). This is the usual case, since the audit runs in the same `pdf` flow that wrote it.
   2. The `html` column recorded for this report in `users/{USER}/data/pdf-index.tsv` (`report \t pdf \t html \t format \t date`, written by `generate-pdf.mjs`). Look the row up by report number; the canonical result is the flat report-linked HTML under `users/{USER}/output/`.
   3. A path the user supplies explicitly.

   Only if none of those resolve, fall back to the newest report-linked `users/{USER}/output/*-{company}-{date}.html` — and say so, because a company with two open roles can still produce several files. Auditing the wrong CV silently is worse than asking. When reading HTML, take the `<li>` items, which the generator emits only for experience and project bullets.
5. **Factual floor** — run `node jd-skill-gap.mjs --user {USER} users/{USER}/jds/{slug}.md --summary` for the zero-LLM classification of every JD requirement into `existing` / `supportedByResume` / `gap`.
...
6. **Scope of truth** — `users/{USER}/cv.md`, `users/{USER}/article-digest.md`, `users/{USER}/config/profile.yml`, `users/{USER}/modes/_profile.md`. These bound what the reviewer may recommend.
...
> [Render in {language.output}: say that no tailored CV was found for {company}; that they should run `/career-ops pdf` first; and that auditing the untailored `users/{USER}/cv.md` instead would produce verdicts on a CV they are not sending. Keep the command literal.]
...
- The candidate's real scope from `users/{USER}/cv.md` and `users/{USER}/article-digest.md`, with this instruction verbatim: *"You may recommend cutting or reframing any bullet. You may never recommend a claim the source files do not support. If a requirement is unmet, say it is unmet — do not invent coverage for it."*
...
Then write this section into `users/{USER}/reports/{num}-{company}-{date}.md`:
...
- **Not on by default.** `pdf.md` Step 20 runs it only for `--hm-audit`, or when `users/{USER}/modes/_custom.md` turns it on for every CV. A `pdf` run that does not ask for it never prompts.
