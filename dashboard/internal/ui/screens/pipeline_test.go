...
// The fork's tab row follows forward funnel progression, where a responded
// posting sits between applied and interview.
func TestRespondedTabSitsBetweenAppliedAndInterview(t *testing.T) {
	...
	if !(applied < responded && responded < interview) {
		t.Fatalf(
			"expected tab order applied < responded < interview, got %d, %d, %d",
			applied, responded, interview,
		)
	}
}
