...
let userContext;
try {
  userContext = getUserContext(argv, { requireUser: targetIdx === -1 });
  // --target is the explicit fixture/non-standard-layout escape hatch used by
  // the doctor tests and external installers. Treat that directory as the
  // user root when no identity was supplied; normal repo runs still require
  // --user and never fall back to root-level candidate data.
  if (!userContext.userRoot) {
    userContext = { ...userContext, userId: 'target', userRoot: projectRoot };
  }
} catch (err) {
...
// project-level MCP config. Some CLIs, including Codex, can also expose these tools
// through runtime tool discovery/lazy loading (for example via tool_search) without a project file. doctor.mjs can inspect
// local files but cannot call the model-side tool registry, so agents must still probe
// their runtime tools before concluding Playwright MCP is unavailable.
...
function checkPlaywrightMcp(root, activeCli) {
  ...
  return {
    ...
    fix: [
      `No project-level MCP config was detected for ${activeCli}; runtime tools may still be available through discovery.`,
      activeCli === 'opencode'
...
function checkScanExtractor(root) {
  const mode = resolveExtractorMode(userPath(userContext, 'config/profile.yml'));
  if (mode === 'cli') {
    ...
    return {
      ...
      fix: [`Restore browser-extract.mjs, or set \`scan.extractor: mcp\` in users/${userContext.userId}/config/profile.yml.`],
    };
  }
...
async function checkPortalSlugs() {
  const portalsPath = userPath(userContext, 'portals.yml');
  if (!existsSync(portalsPath)) {
    return { pass: true, label: `ATS slugs: users/${userContext.userId}/portals.yml not found (skipped)` };
  }
  try {
    ...
    if (unresolved.length === 0) {
      return { pass: true, label: `All ATS slugs in users/${userContext.userId}/portals.yml resolve` };
    }
    return {
      pass: false,
...
function checkPipelineFile() {
  const filePath = userPath(userContext, 'data/pipeline.md');
  if (existsSync(filePath)) {
    return { pass: true, label: `users/${userContext.userId}/data/pipeline.md ready` };
  }
  try {
    writeFileSync(filePath, PIPELINE_SKELETON, 'utf-8');
    return { pass: true, label: `users/${userContext.userId}/data/pipeline.md ready (auto-created)` };
  } catch {
    return {
      pass: false,
      label: `users/${userContext.userId}/data/pipeline.md could not be created`,
      fix: `Run: mkdir -p users/${userContext.userId}/data && touch users/${userContext.userId}/data/pipeline.md`,
    };
  }
}
...
function userPluginRoots() {
  return pluginRoots(projectRoot, userContext.userRoot);
}
...
async function main() {
  ...
  if (STRICT) {
    checks.push(await checkPortalSlugs());
  }
