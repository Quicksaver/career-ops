 ...
 * call — see extractJdSkills()), then classifies each one against the active
 * user's cv.md into
...
try {
  userContext = getUserContext(process.argv.slice(2), {
    requireUser: isDirectExecution && !process.argv.includes('--self-test'),
  });
} catch (err) {
  printUserContextErrorAndExit(err);
}
const CV_PATH = userContext.userRoot ? userPath(userContext, 'cv.md') : '';
...
if (selfTestMode) {
  ...
  if (!existsSync(CV_PATH)) {
    console.error(`Error: active user's CV not found at ${CV_PATH} — create it first.`);
    process.exit(1);
  }
