 ...
 *   node eval-golden.mjs --live   --user <username> --model gpt-4o-mini
 *                                                        # calls openai-eval.mjs (needs key + users/{USER}/cv.md)
...
if (args.includes('--help') || args.includes('-h')) {
  ...
  --live           Call the model live via openai-eval.mjs (needs key + users/{USER}/cv.md)
  --user <id>      Required in live mode; forwarded to openai-eval.mjs
...
if (mode === 'live' && !user) {
  console.error('❌  --user <id> is required in live mode');
  process.exit(1);
}
...
function getCompletion(testCase) {
  ...
  try {
    ...
    const res = spawnSync(process.execPath,
      [join(ROOT, 'openai-eval.mjs'), '--user', user, '--file', jdFile, '--model', model, '--no-save'],
