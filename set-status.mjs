...
const USAGE = `Usage: node set-status.mjs --user <id> <report#|company> <state> [--note "..."] [--role "..."] [--on YYYY-MM-DD] [--force] [--dry-run] [--json]
       node set-status.mjs --user <id> --row N <state> [...]        (explicit tracker row ID)
       node set-status.mjs --user <id> --report N <state> [...]     (explicit report ID)
...
try {
  userContext = getUserContext(process.argv.slice(2), {
    requireUser: !process.env.CAREER_OPS_TRACKER,
  });
} catch (err) {
...
const APPS_FILE = resolveTrackerPath(userContext.userRoot || CAREER_OPS);
