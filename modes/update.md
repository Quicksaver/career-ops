...
Run `node update-system.mjs check --json` and parse the JSON output.
...
- If `dismissed`: Tell the user "Update check was previously dismissed. Clearing the dismissal and re-checking now." Remove `.update-dismissed`, then re-run `node update-system.mjs check --json` and branch on the new status.
...
1. **Read `users/{USER}/modes/_profile.md`** (if it exists)
2. **Diff `modes/_shared.md`**: Run `git diff HEAD..FETCH_HEAD -- modes/_shared.md`
3. **Check for archetype changes**: If `_shared.md` has changes in the "Archetype Detection" section, and `users/{USER}/modes/_profile.md` references archetype names, warn the user:
   > "⚠️ The scoring system or archetypes were updated. Your customizations in `users/{USER}/modes/_profile.md` may reference outdated archetype names. I'll review them after the update."
...
6. Run `node doctor.mjs --user {USER}` to validate the installation
...
7. If Step 3 flagged archetype/scoring changes, reconcile `users/{USER}/modes/_profile.md` against the new `modes/_shared.md`:
   ...
   - Read `users/{USER}/modes/_profile.md` and look for tokens that match archetype names (inline text, Markdown links, YAML keys, code spans).
...
8. Show final status:
   > "✅ Updated to v{version}. Run `node doctor.mjs --user {USER}` anytime to verify setup."
...
- NEVER auto-modify User Layer files during update (`users/{USER}/cv.md`, `users/{USER}/config/profile.yml`, `users/{USER}/data/`, `users/{USER}/reports/`, `users/{USER}/output/`, `users/{USER}/interview-prep/`, `users/{USER}/jds/`, `users/{USER}/article-digest.md`, `users/{USER}/portals.yml`)
- `users/{USER}/modes/_profile.md` is User Layer too: the compatibility check reads it strictly read-only
- Exception: `users/{USER}/modes/_profile.md` may be edited **only** in Step 4.7, and **only** after the user explicitly confirms each individual rename/removal. Never batch-edit without per-change consent.
- User-specific customizations (archetypes, scoring weights, narrative) belong in `users/{USER}/modes/_profile.md` or `users/{USER}/config/profile.yml`, never in `modes/_shared.md`
