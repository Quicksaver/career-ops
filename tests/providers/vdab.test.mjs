...
try {
  ...
  // fetch() — keyword fallback to the active user's profile target_roles.
  {
    ...
    await withTmpCwd(
      ...
      (tmp) => vdab.fetch(
        { name: 'VDAB', vdab: {} },
        {
          profilePath: join(tmp, 'config', 'profile.yml'),
    ...
    try {
      await withTmpCwd(
        ...
        (tmp) => vdab.fetch(
          { name: 'VDAB empty', vdab: {} },
          { ...mkCtx({}), profilePath: join(tmp, 'config', 'profile.yml') },
        ),
      );
