...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    // Fall back to the active user's profile target_roles when this entry has no
    ...
    if (!keywords.length) keywords = resolveProfileKeywords(ctx.profilePath);
    if (!keywords.length) {
      throw new Error(`vdab: entry "${entry.name || '(unnamed)'}" has no vdab.keywords[] and no active-user profile target_roles to fall back to`);
    }
