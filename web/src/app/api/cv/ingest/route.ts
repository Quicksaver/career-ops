...
import { careerOpsCoreEnv, careerOpsSystemRoot } from "@/lib/career-ops";
...
function readCanonicalMode(): string | null {
  try {
    return fs.readFileSync(path.join(careerOpsSystemRoot(), "modes", "cv-ingest.md"), "utf8");
  } catch {
    return null;
  }
}
...
export async function POST(req: Request) {
  ...
  let child;
  try {
    child = spawn(binPath, args, { cwd: careerOpsSystemRoot(), env: careerOpsCoreEnv() });
  } catch (e) {
