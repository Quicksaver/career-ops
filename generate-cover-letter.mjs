...
async function main() {
  ...
  if (args.help || !args.payload) {
    console.log(`
Usage:
  node generate-cover-letter.mjs --user <username> --payload payload.json [--out filename.pdf] [--format letter|a4] [--report NNN]

  --user      Required active career-ops user
...
`);
  ...
  ensureUserDirs(userContext, ["output"]);
  const outputRoot = userPath(userContext, "output");
  ...
  if (!payload.output_path) {
    ...
    payload.output_path = safeOutputPath(payload.output_path, outputRoot);
  }
