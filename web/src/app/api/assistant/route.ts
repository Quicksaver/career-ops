...
import { careerOpsCoreEnv, careerOpsSystemRoot, readMemory, doctorState } from "@/lib/career-ops";
...
- filterPipeline {"tab":"OFFER","min":4,"q":"text","sort":"score","dir":-1} — filter the pipeline table in place. tab ∈ INBOX, ALL, EVALUATED, APPLIED, RESPONDED, INTERVIEW, OFFER, HIRED, REJECTED, CLOSED, DISCARDED, SKIP; min = score floor 0–5.
...
- setStatus {"n":"42","status":"Applied"} — move a tracked application to a new state (asks the user to confirm first). Canonical states: Evaluated, Applied, Responded, Interview, Offer, Hired, Rejected, Closed, Discarded, SKIP. Use the application number (the "#42" on its report page).
...
export async function POST(req: Request) {
  ...
  const child = spawn(binPath, args, { cwd: careerOpsSystemRoot(), env: careerOpsCoreEnv() });
