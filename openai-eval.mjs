 ...
 * user's resume from users/{USER}/cv.md, and evaluates a Job Description
 * passed inline or via --file. Mirrors ollama-eval.mjs / gemini-eval.mjs.
 *
 * Usage:
 *   node openai-eval.mjs --user <username> "Paste full JD text here"
 *   node openai-eval.mjs --user <username> --file ./jds/my-job.txt
 *   node openai-eval.mjs --user <username> --url https://openrouter.ai/api/v1 --model meta-llama/llama-3.3-70b-instruct --file ./jds/job.txt
 *
 ...
 *
 * Privacy: your users/{USER}/cv.md + the full JD are sent to the configured
 * endpoint. Pick a provider you trust; for fully local/private use, run a local
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  USAGE
    node openai-eval.mjs --user <username> "<JD text>"
    node openai-eval.mjs --user <username> --file ./jds/my-job.txt
    node openai-eval.mjs --user <username> --url <base> --model <id> --file ./jds/job.txt
  ...
  EXAMPLES
    OPENAI_API_KEY=sk-... node openai-eval.mjs --user <username> --file ./jds/job.txt
    node openai-eval.mjs --user <username> --url http://localhost:1234/v1 --model local "<JD text>"
`);
  process.exit(0);
}
...
// users/{USER}/cv.md + the full JD (and the API key) are sent to this endpoint, so:
...
const profileYml    = readFile(PATHS.profileYml, `users/${userContext.userId}/config/profile.yml`);
...
console.log(`\n🔒  Privacy: your users/${userContext.userId}/cv.md + JD will be sent to ${endpointHost}.`);
