...
import { careerOpsCoreEnv, careerOpsSystemRoot, rootScript } from "@/lib/career-ops";
 ...
 * a short-lived node process (cwd = the career-ops checkout, with CAREER_OPS_USER
 * selecting the active user root) so the core's own
...
`;
  ...
  return new Promise((resolve) => {
    const child = spawn(process.execPath, ["--input-type=module", "-e", code], {
      cwd: careerOpsSystemRoot(),
      env: careerOpsCoreEnv(),
    });
    let out = "";
    let err = "";
