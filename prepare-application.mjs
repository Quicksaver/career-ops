 ...
 * users/{USER}/config/profile.yml, and prints a prefill summary to stdout.
 * Never POSTs anything — the user reviews the output, opens the apply URL,
 * and submits themselves.
 *
 * Usage:
 *   node prepare-application.mjs --user <id> --url <apply_url> --pdf output/<cv>.pdf
 *   node prepare-application.mjs --user <id> --url <apply_url> --pdf output/<cv>.pdf --cover output/cover.txt
 *
 * Supported ATS:
...
if (!applyUrl || !pdfPath) {
  console.error('Usage: node prepare-application.mjs --user <id> --url <apply_url> --pdf <pdf_path> [--cover <cover_txt>]');
  process.exit(1);
}
...
function readCover() {
  ...
  const abs = resolve(userContext.userRoot, coverPath);
  if (!abs.startsWith(userContext.userRoot + '/') && abs !== userContext.userRoot) {
    console.error(`Warning: cover letter path escapes users/${userContext.userId}/ — skipping`);
    return null;
  }
  if (!existsSync(abs)) {
...
for (const [key, value] of fields) {
  console.log(`  ${key.padEnd(labelWidth)}${value || `(not set — check users/${userContext.userId}/config/profile.yml)`}`);
}
