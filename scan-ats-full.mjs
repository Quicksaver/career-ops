 ...
 *   node scan-ats-full.mjs --user <username> --resume             # continue an interrupted sweep from its checkpoint
...
import { buildTitleFilter, buildLocationFilter, buildContentFilter, matchedTitleKeywords, loadSeenUrls, normalizeUrlForDedup, appendToPipeline, appendToScanHistory, configureScanUserPaths, loadBlacklist, parseSinceDays } from './scan.mjs';
import {
  ensureUserDirs,
  getUserContext,
...
let CACHE_DIR = 'data/cache/ats-companies';
...
let CHECKPOINT_PATH = 'data/cache/ats-full-checkpoint.json';
...
const USAGE = `Usage:
  ...
  node scan-ats-full.mjs --user <username> --resume
...
async function main() {
  ...
  ensureUserDirs(userContext, ['data', 'data/cache/ats-companies']);
  configureScanUserPaths(userContext);
  PORTALS_PATH = process.env.CAREER_OPS_PORTALS
    ? path.resolve(process.env.CAREER_OPS_PORTALS)
    : userPath(userContext, 'portals.yml');
  PIPELINE_PATH = userPath(userContext, 'data/pipeline.md');
  SCAN_HISTORY_PATH = userPath(userContext, 'data/scan-history.tsv');
  CACHE_DIR = userPath(userContext, 'data/cache/ats-companies');
  CHECKPOINT_PATH = userPath(userContext, 'data/cache/ats-full-checkpoint.json');

  const opts = parseArgs(userContext.args);
  let checkpoint = null;
  if (opts.resume) {
