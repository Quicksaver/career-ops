...
- **Check for real reported questions before Block 4.** Reuse `users/{USER}/interview-prep/{company-slug}-{role-slug}.md` if it exists; otherwise run `interview-prep.md`'s Step 1 queries scoped to this round. Follow the same tagging discipline as `interview-prep.md`: sourced with a citation, or `[inferred from JD]` when nothing real turns up.
