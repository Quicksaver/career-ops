...
import { canonicalizeTrackerPath, resolvePdfIndexPath, trackerLockDirFor, acquireTrackerLock, writeFileAtomic, normalizeCompany, cell } from './tracker-utils.mjs';
...
const BATCH_STATE_FILE = process.env.CAREER_OPS_BATCH_STATE
  ...
  : userContext.userRoot
    ? userPath(userContext, 'batch/batch-state.tsv')
...
const MIGRATE_VIA = userContext.args.includes('--migrate-via');
...
const CANONICAL_STATES = ['Evaluated', 'Applied', 'Responded', 'Interview', 'Offer', 'Hired', 'Rejected', 'Closed', 'Discarded', 'SKIP'];
...
for (const file of tsvFiles) {
  ...
  if (duplicate) {
    ...
    // Preserve the existing tracker/report identity. In this fork, row #N,
    // report #N and PDF #N are one compatibility boundary; a re-evaluation may
    // refresh score/content without making the dashboard open another ID.
    ...
    // The full usedNumbers pass also catches rows that the duplicate-detection
    // heuristic skipped, so preserving identity never creates duplicate IDs.
...
if (!DRY_RUN) {
  ...
  let movedFiles = 0;
  for (const file of tsvFiles) {
    if (blockedFiles.has(file) || failedAdditions.has(file)) continue;
...
console.log(`\n📊 Summary: +${added} added, 🔄${updated} updated, ⏭️${skipped} skipped${failedAdditions.size ? `, ❌${failedAdditions.size} NOT merged` : ''}`);
if (DRY_RUN) console.log('(dry-run — no changes written)');
trackerLock.release();
...
if (!DRY_RUN) {
  try {
    const syncArgs = [join(CAREER_OPS, 'sync-pdf-flags.mjs')];
    if (userContext.userId) syncArgs.push('--user', userContext.userId);
    execFileSync('node', syncArgs, {
      stdio: 'inherit',
      env: {
        ...process.env,
        CAREER_OPS_TRACKER: APPS_FILE,
        CAREER_OPS_PDF_INDEX: PDF_INDEX_FILE,
      },
    });
  } catch (e) {
...
if (failedAdditions.size > 0) {
  console.error(
    `\n❌ ${failedAdditions.size} addition(s) were NOT merged and were left in ${ADDITIONS_DIR}: ` +
