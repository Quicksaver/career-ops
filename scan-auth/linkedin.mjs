...
const SELECTORS = {
  ...
  /** Legacy EN UI signal. Kept as a fast path, but not sufficient for localized accounts. */
  loggedIn: 'a[aria-label*="My Network"]',
  /** Stable authenticated nav destinations; text/aria-labels are localized. */
  loggedInNavLinks: [
    'a[href*="/mynetwork"]',
    'a[href*="/jobs/"]',
    'a[href*="/messaging/"]',
  ...
  loginForm: 'input[name="session_key"], input[name="session_password"], form[action*="/uas/login"], form[action*="/login-submit"]',
...
export default class LinkedInScanner {
  ...
  async isLoggedIn(page) {
    ...
    if (await page.$(SELECTORS.loginForm)) return false;
    if (await page.$(SELECTORS.loggedIn)) return true;
    if (await page.$(SELECTORS.loggedInNavLinks)) return true;

    const cookies = typeof page.context === 'function'
      ? await page.context().cookies('https://www.linkedin.com').catch(() => [])
      : [];
    const hasLinkedInAuthCookie = cookies.some(cookie => cookie.name === 'li_at');
    if (!hasLinkedInAuthCookie) return false;

    return Boolean(await page.$('main, .global-nav, nav[aria-label]'));
  }
