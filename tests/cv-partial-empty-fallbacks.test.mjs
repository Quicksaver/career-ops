...
function build(label, templateArg) {
  ...
  const args = [join(ROOT, 'build-cv-html.mjs'), '--user', basename(dir), input, output];
  if (templateArg) args.push(templateArg);

  if (run(NODE, args, { env: { ...process.env, CAREER_OPS_USERS_DIR: dirname(dir) } }) === null) {
...
function blockSuffixedTemplate() {
  // The active-user boundary intentionally permits templates only from the
  // system template tree. Create an isolated pack there and remove it in the
  // test's finally block instead of weakening that production guard.
  const packDir = join(ROOT, 'templates', `.test-cv-2486-${basename(dir)}`);
  generatedPackDir = packDir;
  mkdirSync(packDir, { recursive: true });
  cpSync(join(ROOT, 'templates', 'cv-template.html'), join(packDir, 'cv-template.html'));
  cpSync(join(ROOT, 'templates', 'sections'), join(packDir, 'sections'), { recursive: true });
...
try {
  ...
  if (generatedPackDir) {
    try { rmSync(generatedPackDir, { recursive: true, force: true }); } catch { /* best effort */ }
  }
