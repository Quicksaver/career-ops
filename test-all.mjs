...
if (
  ...
  /userPath\(userContext,\s*['"]cv\.md['"]\)/.test(jdSkillGapSource) &&
  /--user <username>/.test(jdSkillGapSource)
) {
  pass('jd-skill-gap.mjs requires and reads the active user CV');
} else {
  fail('jd-skill-gap.mjs can fall back to a root CV or omit active-user routing');
}

const pasteReplySource = readFile('paste-reply.mjs');
if (
  /getUserContext\(rawArgs/.test(pasteReplySource) &&
  /userPath\(userContext,\s*['"]data\/reply-candidates\.json['"]\)/.test(pasteReplySource) &&
  /requireUser:\s*!CANDIDATES_OVERRIDE/.test(pasteReplySource)
) {
  pass('paste-reply.mjs writes normal CLI data under the active user');
} else {
  fail('paste-reply.mjs can write normal CLI data outside the active user root');
}

const upskillSource = readFile('upskill.mjs');
if (
  /userPath\(userContext,\s*['"]cv\.md['"]\)/.test(upskillSource) &&
  /userPath\(userContext,\s*['"]config\/profile\.yml['"]\)/.test(upskillSource) &&
  !/cv-example\.md/.test(upskillSource)
) {
  pass('targeted upskill uses only active-user CV and profile evidence');
} else {
  fail('targeted upskill can use non-user candidate evidence');
}
...
const criticalRoutingContracts = [
  ...
  ['doctor --json onboarding', /node doctor\.mjs --user \{USER\} --json/],
];
...
try {
  ...
  // flag was read and refused.
  {
    const sinceCli = (...argv) => spawnSync(NODE, [join(ROOT, 'scan.mjs'), ...argv], {
      ...
      timeout: 30000,
      stdio: ['pipe', 'pipe', 'pipe'],
      env: { ...process.env, CAREER_OPS_USER: 'test' },
    });
...
try {
  ...
  try {
    ...
    try {
      cliOut = execFileSync(NODE, [join(ROOT, 'followup-cadence.mjs'), '--json'], {
        cwd: ROOT,
        encoding: 'utf-8',
        timeout: 30000,
        env: { ...TEST_USER_ENV, CAREER_OPS_PROFILE: CADENCE_FIXTURE },
      });
    } catch (cliErr) {
  ...
  // is where a silently-inferred age would actually do damage.
  {
    ...
    try {
      ...
      copyFileSync(join(ROOT, 'lib', 'user-context.mjs'), join(e2eTmp, 'lib', 'user-context.mjs'));
      ...
      mkdirSync(join(e2eTmp, 'users', 'test', 'data'), { recursive: true });
      writeFileSync(join(e2eTmp, 'users', 'test', 'data', 'applications.md'), [
        '# Applications Tracker',
        '',
        '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |',
        '|---|------|---------|------|-------|--------|-----|--------|-------|',
      ...
      const e2eOut = execFileSync(NODE, [join(e2eTmp, 'followup-cadence.mjs'), '--json'], {
        cwd: e2eTmp,
        encoding: 'utf-8',
        timeout: 30000,
        env: {
          ...
          CAREER_OPS_PROFILE: '',
          CAREER_OPS_USER: 'test',
          CAREER_OPS_USERS_DIR: join(e2eTmp, 'users'),
        },
      });
...
try {
  ...
  try {
    const fixtureUser = join(tmpDir, 'test');
    mkdirSync(join(fixtureUser, 'data'), { recursive: true });
    mkdirSync(join(fixtureUser, 'reports'), { recursive: true });
    writeFileSync(join(fixtureUser, 'reports', '012-acme-2026-01-04.md'), '# fixture\n');
    const tracker = join(fixtureUser, 'data', 'applications.md');
    ...
    // Migrate through the active-user resolver against the fixture user root.
    run(NODE, ['merge-tracker.mjs', '--migrate'], { env: { ...process.env, CAREER_OPS_USERS_DIR: tmpDir, CAREER_OPS_USER: 'test' } });
...
try {
  const mergeUsersTmp = mkdtempSync(join(tmpdir(), 'career-ops-merge-users-'));
  try {
    const mergeTmp = join(mergeUsersTmp, 'test');
    mkdirSync(join(mergeTmp, 'data'), { recursive: true });
    mkdirSync(join(mergeTmp, 'reports'), { recursive: true });
    const additionsDir = join(mergeTmp, 'batch', 'tracker-additions');
    ...
    const mergeResult = run(NODE, ['merge-tracker.mjs', '--user', 'test'], { env: { ...process.env, CAREER_OPS_USERS_DIR: mergeUsersTmp } });
    if (mergeResult === null) {
    ...
    rmSync(mergeUsersTmp, { recursive: true, force: true });
  }
} catch (e) {
...
try {
  ...
  try {
    ...
    if (col912Result.status !== 1) {
      fail(`merge-tracker report-number collision guard should exit 1, got ${col912Result.status}`);
    } else {
...
// `addition.num > maxNum` check trusted a colliding number as free. The fork
// builds a Set of every number actually on the tracker (independent of that
// heuristic) and blocks the stale TSV for manual identity repair. It must not
// renumber a worker-reserved report/tracker identity behind the user's back.
...
try {
  ...
  try {
    ...
    if (staleNumResult.status !== 1) {
      fail(`merge-tracker stale-number collision guard should exit 1, got ${staleNumResult.status}`);
    } else {
      const staleNumMerged = readFileSync(staleNumTracker, 'utf-8');
      const staleNumRows = staleNumMerged.split('\n').filter(l => l.startsWith('| ') && !l.startsWith('| #') && !l.startsWith('|---'));

      if (staleNumRows.length === 1) {
        pass('stale-number collision (#1704): conflicting addition is blocked instead of creating a second row');
      } else {
        fail(`stale-number collision (#1704): expected 1 preserved row, got ${staleNumRows.length}`);
      }
      ...
      if (!staleNumRows.some(r => r.includes('NewCo')) && existsSync(join(staleNumAdditions, '001-newco.tsv'))) {
        pass('stale-number collision (#1704): NewCo stays pending for manual ID repair without report-ID drift');
      } else {
        fail(`stale-number collision (#1704): NewCo was merged or its pending TSV was lost\n${staleNumMerged}`);
      }
    }
  } finally {
...
try {
  ...
  try {
    ...
    if (collisionResult.status === 1
        ...
        && /failed integrity guards/i.test(collisionOutput)) {
...
try {
  ...
  try {
    ...
    if (reqResult === null) {
      ...
      if (distinctRow && originalRow1 && originalRow1.includes('R_1000001')) {
      ...
      // Lower-score re-evaluations write through, so the existing row takes the fresher 3.0 score.
      const repostRow = reqRows.find(r => r.includes('(Repost)'));
      if (!repostRow && originalRow1 && originalRow1.includes('3.0/5')) {
        pass('(#1524b) same req number on similar titles → still deduped (lower score written through)');
      } else {
      ...
      // into exactly one row. The lower 3.2 score writes through as the fresher re-evaluation.
      const opsAnalystRows = reqRows.filter(r => r.includes('Operations Analyst'));
      if (opsAnalystRows.length === 1 && opsAnalystRows[0].includes('3.2/5')) {
        pass('(#1524d) req number on only one side → falls back to fuzzy match, still deduped');
      } else {
        fail(`(#1524d) one-sided req number should fall back to fuzzy match: expected 1 'Operations Analyst' row at 3.2/5, got ${opsAnalystRows.length}`);
      }
    }
  } finally {
...
try {
  ...
  function spawnMerge(additionsDir, holdMs = 0) {
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs', '--user', 'test'], {
        cwd: ROOT,
        env: {
          ...process.env,
          CAREER_OPS_USERS_DIR: mergeTmp,
  ...
  try {
    const fixtureUser = join(mergeTmp, 'test');
    mkdirSync(join(fixtureUser, 'data'), { recursive: true });
    mkdirSync(join(fixtureUser, 'reports'), { recursive: true });
    ...
    writeFileSync(join(fixtureUser, 'data', 'applications.md'),
      '# Applications Tracker\n\n' +
      '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |\n' +
      '|---|------|---------|------|-------|--------|-----|--------|-------|\n');
    writeFileSync(join(fixtureUser, 'reports', '010-alpha-2026-01-07.md'), '# fixture\n');
    writeFileSync(join(fixtureUser, 'reports', '011-beta-2026-01-07.md'), '# fixture\n');
    ...
    await waitForReady(first.ready, 10_000);
    ...
    const merged = readFileSync(join(fixtureUser, 'data', 'applications.md'), 'utf-8');
...
try {
  ...
  rmSync(virginUsers, { recursive: true, force: true });

  // Fully provisioned env: all 4 present → must NOT onboard.
  const readyUsers = mkdtempSync(join(tmpdir(), 'co-ready-users-'));
  const ready = join(readyUsers, 'test');
  ...
  const r = JSON.parse(run(NODE, ['doctor.mjs', '--user', 'test', '--json'], { env: { ...process.env, CAREER_OPS_USERS_DIR: readyUsers } }) || '{}');
  ...
  rmSync(readyUsers, { recursive: true, force: true });
  ...
  if (
    /node\s+doctor\.mjs\s+--user\s+\{USER\}\s+--json/.test(agentsDoc) &&
    /"warnings"\s*:\s*\[\.\.\.\]/.test(agentsDoc) &&
    /"autoCopied"\s*:\s*\[\.\.\.\]/.test(agentsDoc) &&
    !/Does\s+`cv\.md`\s+exist\?/i.test(agentsDoc) &&
    claudeWrapperLines[0] === '@AGENTS.md' &&
    ...
    fail('AGENTS.md misses user-scoped onboarding state docs or CLAUDE.md is not a thin wrapper');
  }
} catch (e) {
...
try {
  ...
  const noMcpUsers = mkdtempSync(join(tmpdir(), 'co-nomcp-users-'));
  ...
  const withMcpUsers = mkdtempSync(join(tmpdir(), 'co-mcp-users-'));
  mkdirSync(join(withMcpUsers, 'test'), { recursive: true });
  ...
  const b = JSON.parse(run(NODE, ['doctor.mjs', '--user', 'test', '--json', '--target', withMcp], { env: { ...process.env, CAREER_OPS_USERS_DIR: withMcpUsers } }) || '{}');
...
try {
  ...
  const userBatchDir = join(userRoot, 'batch');
  const fakeBin = join(tmp, 'bin');
  mkdirSync(batchDir, { recursive: true });
  mkdirSync(userBatchDir, { recursive: true });
  mkdirSync(join(userRoot, 'reports'), { recursive: true });
  ...
  writeFileSync(join(userBatchDir, 'batch-input.tsv'), [
    'id\turl\tsource\tnotes',
  ...
  const env = { ...process.env, PATH: `${fakeBin}${delimiter}${process.env.PATH}`, CAREER_OPS_USERS_DIR: usersDir };
  const out = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1', '--max-retries', '3', '--rate-limit-sleep', '0'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
  const state = readFileSync(join(userBatchDir, 'batch-state.tsv'), 'utf-8').trim().split('\n');
  ...
  writeFileSync(join(userBatchDir, 'batch-state.tsv'), [
  ...
  const dry = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--resume-paused', '--dry-run'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
  ...
  const statusOnly = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--status'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
...
try {
  ...
  const out = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1'], { cwd: tmp, env, stdio: ['pipe', 'pipe', 'pipe'] }) || '';
...
try {
  ...
  const argFile = join(tmp, 'claude-argv.txt');
  const env = { ...process.env, PATH: `${fakeBin}${delimiter}${process.env.PATH}`, BATCH_ARG_FILE: argFile };
  const premiumOut = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1'], { cwd: tmp, env, stdio: ['pipe', 'pipe', 'pipe'] }) || '';
...
try {
  ...
  const argFile = join(tmp, 'claude-argv.txt');
  const env = { ...process.env, PATH: `${fakeBin}${delimiter}${process.env.PATH}`, BATCH_ARG_FILE: argFile };
  const overrideOut = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1', '--model', 'claude-sonnet-5'], { cwd: tmp, env, stdio: ['pipe', 'pipe', 'pipe'] }) || '';
...
try {
  ...
  const argFile = join(tmp, 'claude-argv.txt');
  const env = { ...process.env, PATH: `${fakeBin}${delimiter}${process.env.PATH}`, BATCH_ARG_FILE: argFile };
  const standardDefaultOut = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1'], { cwd: tmp, env, stdio: ['pipe', 'pipe', 'pipe'] }) || '';
...
try {
  ...
  const argFile = join(tmp, 'claude-argv.txt');
  const env = { ...process.env, PATH: `${fakeBin}${delimiter}${process.env.PATH}`, BATCH_ARG_FILE: argFile };
  const invalidTierOut = run(getBash(), [toBashPath(join(batchDir, 'batch-runner.sh')), '--user', 'test', '--parallel', '1'], { cwd: tmp, env, stdio: ['pipe', 'pipe', 'pipe'] }) || '';
...
try {
  ...
  const succUserRoot = join(succTmp, 'users', 'alice');
  const userLocalGmail = join(succUserRoot, 'plugins.local', 'gmail');
  mkdirSync(userLocalGmail, { recursive: true });
  writeFileSync(join(userLocalGmail, 'manifest.json'), JSON.stringify({ id: 'gmail', apiVersion: 1, description: 'user-scoped successor gmail', hooks: ['ingest'], requiredEnv: [], allowedHosts: [], humanInTheLoop: true }));
  writeFileSync(join(userLocalGmail, 'index.mjs'), 'export default { ingest: async () => [] };');
  lockMod.writeLockEntry(succUserRoot, 'gmail', { source: 'local', sha: SUCC_SHA, version: '2.0.0', integrity: 'x', files: {}, consent: {} });
  const userIds = eng.resolveSuccessorIds(succTmp, succUserRoot);
  const userDisc = eng.discoverPlugins(eng.pluginRoots(succTmp, succUserRoot), userIds).find(m => m.id === 'gmail');
  if (userIds.has('gmail') && userDisc?.dir === userLocalGmail) pass('successor: approved successor resolution uses the active user root for plugins.local and plugins.lock');
  else fail('successor: user-scoped approved successor should override bundled reference from the active user root');
