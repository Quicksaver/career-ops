...
(`users/{USER}/data/agent-inbox.md`) that any tool — this CLI, a dashboard button, a cron job,
...
```bash
node agent-inbox.mjs --user {USER} add "evaluate https://acme.com/jobs/42"
node agent-inbox.mjs --user {USER} add "draft a follow-up for application #7"
node agent-inbox.mjs --user {USER} add "run a scan and triage anything new"
```

## Inspect / resolve

```bash
node agent-inbox.mjs --user {USER} list            # pending items
node agent-inbox.mjs --user {USER} list --all      # include resolved items
node agent-inbox.mjs --user {USER} resolve 1 --result "scored 4.3 — report 012"
```

`users/{USER}/data/agent-inbox.md` is user-layer (gitignored). Items look like:
...
1. Read `users/{USER}/data/agent-inbox.md`. If it doesn't exist or has no unchecked items,
   say so and stop.
...
3. After each, mark it `[x]` and append `→ result: <one line>` — either by hand
   or with `node agent-inbox.mjs --user {USER} resolve <n> --result "..."`.
