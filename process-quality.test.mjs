...
function runUserCli(args) {
  const dir = mkdtempSync(join(tmpdir(), 'process-quality-users-'));
  try {
    mkdirSync(join(dir, 'test', 'data'), { recursive: true });
    return execFileSync('node', [scriptPath, '--user', 'test', ...args], {
      encoding: 'utf-8',
      timeout: 10000,
      cwd: dirname(scriptPath),
      env: { ...process.env, CAREER_OPS_USERS_DIR: dir },
    });
  } finally {
...
const summaryOut = runUserCli(['--summary']);
