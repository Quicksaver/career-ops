 ...
 *
 * Usage:
 *   node outcome.mjs --user USER <report#|company> <outcome_type> [--stage "..."] [--feedback "..."] [--note "..."] [--role "..."] [--cv "..."] [--cover "..."] [--dry-run] [--json]
 *
 * Outcomes:
...
let userContext;
try {
  userContext = getUserContext(originalArgs, { requireUser: !process.env.CAREER_OPS_TRACKER });
} catch (err) {
  printUserContextErrorAndExit(err);
}
const rawArgs = userContext.args;
const positional = [];
const flags = {
  stage: null,
  feedback: null,
...
const appsFile = process.env.CAREER_OPS_TRACKER
...
if (!resolvedPostingPath && targetUrl) {
  try {
    const archiveArgs = [ARCHIVE_POSTING_SCRIPT];
    if (userContext.userId) archiveArgs.push('--user', userContext.userId);
    archiveArgs.push(targetUrl, `--company=${matchedRow.company}`, `--role=${matchedRow.role}`);
    execFileSync(NODE, archiveArgs, {
      cwd: CAREER_OPS,
      env: process.env,
      stdio: 'ignore',
      timeout: 45000,
...
if (userContext.userId) setStatusArgs.splice(1, 0, '--user', userContext.userId);
