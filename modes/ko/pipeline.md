...
1. **Playwright (preferred):** `browser_navigate` + `browser_snapshot`. 모든 SPA에 대응합니다.
   - **선택 — CLI 추출기 (`users/{USER}/config/profile.yml`의 `scan.extractor: cli`):** 대신 `node browser-extract.mjs <url>`(`--mode jd`) 실행 — 간결한 `{ "url", "title", "text" }`, 더 적은 토큰(포털에 따라 다름). 오류나 부재 시 **조용히** `browser_navigate` + `browser_snapshot`로 폴백.
