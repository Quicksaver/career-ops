...
try {
  ...
  if (/\[7\]/.test(downRow) && !/\[8\]\(/.test(downRow)) {
    pass('downgrade preserves the existing tracker/report identity');
  } else {
    fail(`downgrade changed the canonical report identity: ${downRow.trim()}`);
  }

  // The fork keeps row #N, report #N and PDF #N aligned. Since the old report
  // is not superseded in the row, no contradictory superseded marker belongs
  // in Notes.
  if (!/Superseded report/.test(downRow)) {
    pass('downgrade does not contradict the preserved report identity');
  } else {
    fail(`downgrade marked the retained report as superseded: ${downRow.trim()}`);
  }
  ...
  // Equal scores write through too: notes are fresher, while the canonical
  // report identity stays aligned with the tracker row.
  ...
  if (/\[7\]/.test(sameRow) && !/\[10\]\(/.test(sameRow) && /Re-eval 2026-03-02 \(3\.8→3\.8\)/.test(sameRow)
...
try {
  ...
  const previousUser = process.env.CAREER_OPS_USER;
  const previousUsersDir = process.env.CAREER_OPS_USERS_DIR;
  process.env.CAREER_OPS_USER = 'test';
  process.env.CAREER_OPS_USERS_DIR = join(tmpdir(), `career-ops-merge-test-users-${process.pid}`);
  ...
  if (previousUser === undefined) delete process.env.CAREER_OPS_USER;
  else process.env.CAREER_OPS_USER = previousUser;
  if (previousUsersDir === undefined) delete process.env.CAREER_OPS_USERS_DIR;
  else process.env.CAREER_OPS_USERS_DIR = previousUsersDir;
...
// score. This fork preserves report/tracker identity instead of silently
// renumbering: the first row lands and the colliding TSV stays pending for
// repair under a genuinely free reservation.
...
try {
  ...
  if (collisionRows.length === 1
    ...
    pass('a same-run reserved-number collision does not fold distinct roles');
  } else {
    fail(`same-run num collision folded distinct roles: ${collisionRows.join(' // ')}`);
  }
  if (collision.exitCode !== 0
      && collision.pending.includes('051-acme-b.tsv')
      && collision.archived.includes('050-acme-a.tsv')) {
    pass('the colliding TSV stays pending and the merge fails instead of renumbering it');
  } else {
    fail(`same-run num collision did not preserve the pending repair boundary: ${JSON.stringify(collision)}`);
  }
