...
Lees `users/{USER}/cv.md`. Maak een tabel waarin elke voorwaarde van de vacature wordt weergegeven op de exacte regels van het CV.
...
**Verhalenbank:** Als `users/{USER}/interview-prep/story-bank.md` bestaat, controleer dan of deze verhalen er al zijn. Voeg anders het nieuws toe. In de loop van de tijd bouwt dit een herbruikbare bank van 5-10 meesterverhalen op die aanpasbaar zijn aan elke interviewvraag.
...
Sla de volledige evaluatie op in `users/{USER}/reports/{###}-{company-slug}-{YYYY-MM-DD}.md`.

- `{###}` = volgend volgnummer (3 cijfers, nullen opgevuld). Om het atomair toe te wijzen en racecondities te vermijden, zou je `node reserve-report-num.mjs --user {USER}` uitvoeren om het nummer te reserveren (stdout retourneert `{###}`), het rapport schrijven en vervolgens `node reserve-report-num.mjs --user {USER} --release {###}` uitvoeren om de sentinel vrij te geven.
...
**ALTIJD** registreren via een TSV-bestand in `users/{USER}/batch/tracker-additions/`; bewerk `users/{USER}/data/applications.md` nooit rechtstreeks. Voer vóór voltooiing, in deze volgorde, `node merge-tracker.mjs --user {USER}`, `node verify-pipeline.mjs --user {USER}`, `node normalize-statuses.mjs --user {USER}` en `node dedup-tracker.mjs --user {USER}` uit:
...
- Rapport: relatieve link naar het rapportbestand (bijvoorbeeld: `[001](users/{USER}/reports/001-company-2026-01-01.md)`)
- Notities: optioneel
