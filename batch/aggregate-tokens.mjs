...
function main(userContext) {
  const stateFile = userPath(userContext, 'batch/batch-state.tsv');
  const logsDir = userPath(userContext, 'batch/logs');
  ...
  for (const line of lines.slice(1)) {
    ...
    const logFile = path.join(logsDir, `${safeReportNum}-${safeId}.log`);
    const resolvedLogFile = path.resolve(logFile);
    if (!resolvedLogFile.startsWith(path.resolve(logsDir) + path.sep)) continue;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    main(getUserContext(process.argv.slice(2)));
  } catch (err) {
