...
const TEST_USERS_DIR = mkdtempSync(join(tmpdir(), 'career-ops-doctor-billing-'));

/** Run doctor and return its human output (it exits non-zero when it finds issues). */
function runDoctor(env) {
  try {
    return execFileSync(NODE, [DOCTOR, '--user', 'test'], {
      cwd: ROOT,
      env: { ...process.env, CAREER_OPS_USERS_DIR: TEST_USERS_DIR, ...env },
      encoding: 'utf-8',
...
process.on('exit', () => rmSync(TEST_USERS_DIR, { recursive: true, force: true }));
