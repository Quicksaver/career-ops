...
try {
  userContext = getUserContext(process.argv.slice(2), {
    requireUser: IS_MAIN && !process.argv.includes('--self-test'),
  });
} catch (err) {
...
export function loadBenchmarks(explicitPath) {
  const path = explicitPath
    || (userContext.userRoot && existsSync(userPath(userContext, 'config/benchmarks.yml'))
      ? userPath(userContext, 'config/benchmarks.yml')
      : join(CAREER_OPS, 'templates/benchmarks.yml'));
  let doc;
  try {
...
function main() {
  ...
  const states = loadCanonicalStates(STATES_FILE);
  const trackerPath = resolveTrackerPath(userContext.userRoot);
