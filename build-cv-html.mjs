...
//
// The agent reads users/{USER}/cv.md + users/{USER}/config/profile.yml, tailors the content, and writes a
...
async function main() {
  ...
  if (rawArgs.length === 0 || rawArgs.includes('--help')) {
    ...
    console.error('  printed by `node cv-templates.mjs --user <id> resolve cv` to use a selected template.');
    console.error('');
  ...
  if (relOut === '' || relOut.startsWith('..') || isAbsolute(relOut)) {
    console.error(`Refusing to write the HTML file outside the active user root: ${absOutput}`);
    process.exit(1);
  }

  const templatesRoot = resolve(__dirname, 'templates');
  const relTemplate = relative(templatesRoot, templatePath);
  if (relTemplate === '' || relTemplate.startsWith('..') || isAbsolute(relTemplate)) {
    console.error(`Refusing to read a template outside the system templates directory: ${templatePath}`);
    process.exit(1);
  }
...
async function runSelfTest() {
  ...
  const photoPayload = structuredClone(sample);
  photoPayload.candidate.photo = 'data:image/png;base64,AA==';
  const photoHtml = renderHtml(template, photoPayload, TEMPLATE_PATH);
  if (!photoHtml.includes('src="data:image/png;base64,AA=="')) {
    console.error('Self-test failed: supported image data URL was not preserved');
    process.exit(1);
  }
  photoPayload.candidate.photo = 'javascript:alert(1)';
  const unsafePhotoHtml = renderHtml(template, photoPayload, TEMPLATE_PATH);
  if (unsafePhotoHtml.includes('javascript:') || unsafePhotoHtml.includes('<img class="cv-photo"')) {
    console.error('Self-test failed: unsafe photo scheme was not rejected');
    process.exit(1);
  }
