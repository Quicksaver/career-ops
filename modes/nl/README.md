...
Voeg in `users/{USER}/config/profile.yml` toe:
...
Herinner Claude hieraan tijdens je eerste sessie ("Kijk in `users/{USER}/config/profile.yml`, ik heb `language.modes_dir` geconfigureerd"). Claude gebruikt automatisch de Nederlandse modi.
...
- `users/{USER}/cv.md`, `pipeline`, `tracker`, `report`, `score`, `archetype`
