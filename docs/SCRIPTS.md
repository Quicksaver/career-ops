...
Confirmed tracker duplicates preserve lifecycle order as `Hired > Offer > Interview > Responded > Applied > Rejected > Evaluated > Skip/Closed`. The reviewer selects the canonical tracker/report identity, while the deterministic resolver selects the most advanced lifecycle source. If that source is `Applied` or later and is not the canonical row, its report and existing HTML/PDF are backed up, renamed into the canonical keeper filenames, and kept active; the superseded canonical artifacts are archived. The surviving tracker row receives the promoted status, canonical PDF/report links, and merged notes. `data/duplicate-resolutions.jsonl` records both the canonical keeper and artifact source.
