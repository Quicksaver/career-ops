...
// No network is involved.
{
  ...
  try {
    ...
    writeFileSync(join(userRoot, 'data', 'applications.md'), `# Applications Tracker

| # | Date | Company | Role | Score | Status | PDF | Report | Notes |
|---|------|---------|------|-------|--------|-----|--------|-------|
`);
    writeFileSync(join(userRoot, 'data', 'pipeline.md'), '# Pipeline\n\n');
...
`);
    ...
    const scan = () => execFileSync(NODE, [join(ROOT, 'scan.mjs'), '--user', 'test'], {
      cwd: dir,
      env: { ...process.env, CAREER_OPS_USERS_DIR: usersDir, CAREER_OPS_PORTALS: portals },
      encoding: 'utf-8',
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    const entries = () => {
      const p = join(userRoot, 'data', 'pipeline.md');
