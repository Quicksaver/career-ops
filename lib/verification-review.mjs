  ...
  for (const decision of review.findings) {
    ...
    if (['restore_orphan', 'archive_orphan'].includes(decision.disposition)) {
      if (decision.classification !== 'confirmed_orphan' || finding.code !== 'orphan_report' || !decision.orphan_resolution) {
        ...
        if (decision.disposition === 'restore_orphan' &&
            !/^batch\/tracker-additions\/merged\/[A-Za-z0-9._-]+\.tsv$/.test(resolution.tracker_tsv || '')) {
          errors.push(`${key}: restore_orphan tracker_tsv must be user-root-relative under batch/tracker-additions/merged`);
        }
      }
...
export function validateDuplicateConsistency(verification, review) {
  const findings = verificationFindings(verification);
  const findingsByKey = new Map(findings.map(item => [reviewKey(item), item]));
  ...
  for (const decision of (review?.findings || []).filter(item =>
    ...
    const finding = findingsByKey.get(reviewKey(decision));
    const entries = finding?.details?.entries || [];
    const reportFiles = entries.map(entry => reportBasename(entry.report)).filter(Boolean);
    if (new Set(reportFiles).size < 2) continue;

    // Orphaned reports can make a report warning broader than the corresponding
    // tracker warning. A judgment about that whole superset does not determine
    // whether the exact tracker-backed subset is duplicate.
    const matching = reportFindings.find(candidate => {
      const files = (candidate.details?.files || []).map(file => String(file).split('/').pop());
      return sameMembers(reportFiles, files);
    });
    if (!matching) continue;

    const reportDecision = decisionsByKey.get(reviewKey(matching));
    if (!reportDecision || reportDecision.disposition !== 'resolve_duplicate') {
      errors.push(`${decision.finding_id} conflicts with non-duplicate report decision ${matching.id}`);
      continue;
    }
    const keeperEntry = entries.find(entry =>
      entry.tracker_num === decision.duplicate_resolution?.keeper_tracker_num);
    ...
    if (expectedKeeper && selectedKeeper !== expectedKeeper) {
      errors.push(`${matching.id}: report keeper must match tracker keeper #${decision.duplicate_resolution.keeper_tracker_num}`);
    }
  }
  if (errors.length > 0) {
    throw new Error(`duplicate consistency validation failed:\n- ${errors.join('\n- ')}`);
  }
}
