...
import { careerOpsCoreEnv, careerOpsRoot, careerOpsSystemRoot, rootScript } from "@/lib/career-ops";
...
export async function GET() {
  ...
  const stdout = await new Promise<string>((resolve) => {
    execFile(
      "node",
      [verifyPortals],
      { cwd: careerOpsSystemRoot(), env: careerOpsCoreEnv(), timeout: 110_000, maxBuffer: 4 * 1024 * 1024 },
