...
function usage() {
  return 'Usage: node application-artifacts.mjs --user ID --report N --company NAME --role ROLE [--version N] [--root PATH] [--init]';
}
...
async function main() {
  ...
  const root = values.root ? resolve(values.root) : userPath(userContext, 'output');
  const paths = applicationArtifactPaths({ reportNum: values.report, company: values.company, role: values.role, version: values.version, root });
