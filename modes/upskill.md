...
- `users/{USER}/data/applications.md` — Application tracker (rows with report links)
- `users/{USER}/reports/` — Evaluation reports (Machine Summary + Gap tables)
- `users/{USER}/cv.md` + `users/{USER}/config/profile.yml` — Known skills (a skill present here must NEVER appear as a gap)
- `users/{USER}/data/upskill/report-*.md` — Previous upskill reports (for the diff section)
...
| `excludedAsKnown` | Skills found in report gaps but already present in `users/{USER}/cv.md`/`users/{USER}/config/profile.yml` |
...
Write to `users/{USER}/data/upskill/report-{YYYY-MM-DD}.md` (user layer — never touched by the updater). Create the `users/{USER}/data/upskill/` directory if missing.
...
Find the newest existing `users/{USER}/data/upskill/report-*.md` (by filename date) from before today.
...
> "If you've since gained any of these skills, tell me — I'll add them to `users/{USER}/cv.md`/`users/{USER}/config/profile.yml`, and the next run will show the gap closing."

## Rules

- **Output is user layer** (`users/{USER}/data/upskill/`) — never write gap analysis into system files.
- **A skill present in `users/{USER}/cv.md`/`users/{USER}/config/profile.yml` never appears as a gap.** If the user disputes an exclusion, fix the source files, not the report.
