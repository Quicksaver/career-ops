...
Before the first phase, the runner prints `run-id: <RUN_ID>` and its `logs:` directory so an interrupted run can be resumed with `--resume-run <RUN_ID>`. After each call completes, it writes one compact stdout line per finding: `reviewed X/Y, job(s) #{related IDs}, {issue code} → {classification}`. Tracker IDs are preferred; report IDs are used for report-only and orphan findings. Any still-active items classified for `manual_review` are repeated at the very end in the same compact job/code/classification form, so the operator does not have to search earlier progress output. Full decisions, rationale, evidence, and action details remain in the run artifacts instead of flooding the terminal while a run is incomplete. A `completed` run is immediately compacted to one `summary.json`; every other status retains the detailed artifacts. The default final output is only a compact summary plus the actual failure text, when present. Pass `--json` for a complete machine-readable result; in that mode stdout contains only JSON and progress moves to stderr, with no human-review recap added. `--quiet` suppresses live progress while retaining phase logs for non-completed runs and the run path in the final human summary.
