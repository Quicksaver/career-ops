...
> Apply `users/{USER}/voice-dna.md` (if present) to free-text answers and cover-letter fields — full guardrail, conversational voice included (Tier 1 + Tier 2). See `_shared.md` → Voice DNA.
...
```bash
node application-answers.mjs --user {USER} --report reports/NNN-company-role-date.md --input answers.json --state filled
```

## Step 9 — Post-apply (optional)

If the candidate confirms that they submitted the application:
1. Update status through the canonical CLI: `node set-status.mjs --user {USER} <report#> Applied` (never hand-edit the table)
2. Seed the follow-up schedule: run `node followup-seed.mjs --user {USER} {num} --json` (where `{num}` is the tracker row number). If the candidate applied on a different day than today, pass `--date YYYY-MM-DD` with the actual submission date. It's idempotent, so re-running is safe.
