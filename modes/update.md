...
3. Run `node doctor.mjs --user {USER}` to validate the installation
...
4. If Step 3 flagged archetype/scoring changes, reconcile `users/{USER}/modes/_profile.md` against the new `modes/_shared.md`:
...
- `users/{USER}/modes/_profile.md` is User Layer too: the compatibility check in Step 3 reads it strictly read-only
- Exception: `users/{USER}/modes/_profile.md` may be edited **only** in Step 4.3, and **only** after the user explicitly confirms each individual rename/removal. Never batch-edit without per-change consent.
