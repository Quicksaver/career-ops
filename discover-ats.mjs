...
let PORTALS_PATH = process.env.CAREER_OPS_PORTALS || '';
...
const USAGE = `Usage:
  node discover-ats.mjs --user <id> --in companies.yml            # PREVIEW — resolve + print entries, write nothing
  node discover-ats.mjs --user <id> --in companies.yml --write    # opt in: append resolved entries to portals.yml
  node discover-ats.mjs --user <id> Stripe Ramp Mollie            # company names as positional args
  node discover-ats.mjs --user <id> --in companies.yml --summary  # human-readable table
  node discover-ats.mjs --user <id> --in companies.yml --vendors gh,ashby,lever
  node discover-ats.mjs --user <id> --in companies.yml --vendors workday
...
async function main() {
  ...
  if (rawArgs.includes('--self-test')) runSelfTest();

  let userContext;
  try {
    userContext = getUserContext(rawArgs, {
      requireUser: !rawArgs.includes('--help') && !rawArgs.includes('-h'),
    });
  } catch (err) {
    printUserContextErrorAndExit(err);
  }
  const opts = parseArgs(['node', 'discover-ats.mjs', ...userContext.args]);
  PORTALS_PATH = process.env.CAREER_OPS_PORTALS || userPath(userContext, 'portals.yml');
