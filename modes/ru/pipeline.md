...
1. **Playwright (предпочтительно):** `browser_navigate` + `browser_snapshot`. Работает со всеми SPA.
   - **Опционально — CLI-экстрактор (`scan.extractor: cli` в `users/{USER}/config/profile.yml`):** вместо этого запустите `node browser-extract.mjs <url>` (`--mode jd`) — компактный `{ "url", "title", "text" }`, меньше токенов (зависит от портала). При ошибке или отсутствии **молча** откатывайтесь на `browser_navigate` + `browser_snapshot`.
