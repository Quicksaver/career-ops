...
let userContext;
try {
  userContext = getUserContext(process.argv.slice(2), { requireUser: isMain && !process.env.CAREER_OPS_TRACKER });
} catch (err) {
  printUserContextErrorAndExit(err);
}

const MD_PATH = process.env.CAREER_OPS_TRACKER || userPath(userContext, 'data/applications.md');
...
const STATES_PATH = systemPath('templates/states.yml');
...
async function main() {
  const [command, ...args] = userContext.args;
