...
const DEFAULT_PROFILE_PATH = process.env.CAREER_OPS_PROFILE || '';
...
if (isMain) {
  ...
  try {
    if (cmd === 'list') {
      ...
      const profilePath = process.env.CAREER_OPS_PROFILE || userPath(userContext, 'config/profile.yml');
      process.stdout.write(resolveTemplate(kind, name, { format, fallback: Boolean(flags.fallback), profilePath }) + '\n');
    } else {
      process.stderr.write('Usage: node cv-templates.mjs --user <id> <list|resolve> <cv|cover> [name] [--format=html|tex] [--fallback]\n');
      process.exit(2);
    }
  } catch (err) {
