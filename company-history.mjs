  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (normalized === 'applied') {
      ...
      facts.push({
        ...
        clearInstruction: `if they actually responded, node set-status.mjs --user {USER} ${row.num} <state> --note "responded <date>" clears this`,
      });
...
export function renderSummary(result) {
  ...
  if (aged.length > 0) {
    lines.push(`  ${aged.length} aged-Applied row(s) look silent — confirm real or update (node set-status.mjs --user {USER} <num> <state> --note "responded <date>").`);
    lines.push('');
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const bypassUser = rawArgs.includes('--self-test') || rawArgs.includes('--help') || rawArgs.includes('-h');
  let userContext;
  if (!bypassUser) {
    try {
  ...
  if (selfTestMode) {
    ...
    const run = async () => {
      const userRoot = userContext.userRoot;
      const tracker = loadTrackerRows(userRoot);
      const followups = loadFollowupRows(userRoot, followupsOverride);
      const scanHistory = loadRepostClusters(userRoot, scanHistoryOverride);
      const statusLog = await loadStatusLogSource();
