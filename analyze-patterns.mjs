...
function analyze() {
  ...
  const enriched = entries.map(e => {
    ...
    // merge-tracker.mjs link normalization); fall back to the active user root
    // for legacy user-root-relative links.
    let reportPath = null;
    if (reportMatch) {
      const fromTracker = join(dirname(APPS_FILE), reportMatch[1]);
      const candidate = existsSync(fromTracker) ? fromTracker : join(userContext.userRoot, reportMatch[1]);
      const reportsRelative = relative(userPath(userContext, 'reports'), candidate).split(sep).join('/');
      if (reportsRelative !== '..' && !reportsRelative.startsWith('../')) {
  ...
  if (topDiscardReason && topDiscardReason.frequency >= Math.max(3, Math.ceil(enriched.length * 0.15))) {
    recommendations.push({
      action: `Add "${topDiscardReason.reason}" filter to users/${userContext.userId}/modes/_custom.md to avoid wasting evaluation effort`,
