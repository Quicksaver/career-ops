...
1. **Playwright (prefere) :** `browser_navigate` + `browser_snapshot`. Fonctionne avec toutes les SPAs.
   - **Option — extracteur CLI (`scan.extractor: cli` dans `users/{USER}/config/profile.yml`) :** exécutez plutôt `node browser-extract.mjs <url>` (`--mode jd`) — `{ "url", "title", "text" }` compact, moins de tokens (selon le site). **Repli silencieux** vers `browser_navigate` + `browser_snapshot` en cas d'erreur ou d'absence.
