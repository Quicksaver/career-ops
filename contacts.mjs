 ...
 *
 * Run: node contacts.mjs --user USER                     (JSON: contacts + quality + total)
 *      node contacts.mjs --user USER --summary           (human-readable table)
 *      node contacts.mjs --user USER --vcf [path]        (write vCard under the user's output/)
...
function writeVcf(contacts, quality) {
  ...
  const outPath = vcfPathArg ? resolve(ACTIVE_USER_ROOT, vcfPathArg) : DEFAULT_VCF;
  ...
  // active user. Anchored to the user root, not process.cwd() — see the
  // generate-pdf.mjs precedent. Cheap lexical gate first…
  const relOut = relative(ACTIVE_USER_ROOT, outPath);
  ...
  const repoReal = realpathSync(ACTIVE_USER_ROOT);
...
function main() {
  if (rawArgs.includes('--self-test')) { selfTest(); return; }
  ...
  ACTIVE_USER_ROOT = userContext.userRoot;
  CONTACTS_PATH = userPath(userContext, 'data/contacts.tsv');
  DEFAULT_VCF = userPath(userContext, 'output/contacts.vcf');
  summaryMode = args.includes('--summary');
  callerIdMode = args.includes('--caller-id');
  const vcfIdx = args.indexOf('--vcf');
  vcfMode = vcfIdx !== -1;
  // Optional path argument: the token right after --vcf, unless it is another flag.
  vcfPathArg = vcfMode && args[vcfIdx + 1] && !args[vcfIdx + 1].startsWith('--') ? args[vcfIdx + 1] : null;
