...
try {
  ...
  const terminal = stats.computeFunnel({ Hired: 1, Closed: 4 });
  if (terminal.everApplied === 1 && terminal.everResponded === 1 && terminal.everInterview === 1 && terminal.everOffer === 1) {
    pass('computeFunnel counts Hired through every reached stage and excludes pre-application Closed rows');
  } else {
    fail(`computeFunnel terminal-state math wrong: ${JSON.stringify(terminal)}`);
  }
  ...
  const summaryOut = run(NODE, [join(ROOT, 'stats.mjs'), '--user', 'test', '--summary'], { env: cliEnv });
