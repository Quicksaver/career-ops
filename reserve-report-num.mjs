 ...
 * ## Usage
 *
 *   node reserve-report-num.mjs --user <username>
 *   # stdout: 035           (zero-padded, 3 digits)
 *
 *   node reserve-report-num.mjs --user <username> --count 8
 ...
 *
 *   node reserve-report-num.mjs --user <username> --release 035
 *   node reserve-report-num.mjs --user <username> --release 042-049
 *   # Deletes the sentinel(s) (call after writing the real report(s)).
 *
 *   node reserve-report-num.mjs --user <username> --gc
...
let REPORTS_DIR = process.env.CAREER_OPS_REPORTS_DIR || process.env.CAREER_OPS_REPORTS || '';

if (REPORTS_DIR) {
  // Test fixtures and isolated harnesses can point reservations at a temp dir
  // without needing to construct a full users/{USER}/ tree.
  try {
    cliArgs = getUserContext().args;
  } catch {
    // No active user is fine when an explicit reports dir is provided.
  }
} else {
  let userContext;
  try {
  ...
  cliArgs = userContext.args;
  REPORTS_DIR = userPath(userContext, 'reports');
}
...
const [cmd, arg] = cliArgs;

if (cmd === '--release') {
  const m = (arg || '').match(/^(\d+)(?:-(\d+))?$/);
  if (!m) {
    process.stderr.write('Usage: node reserve-report-num.mjs --user <username> --release <NNN>[-<MMM>]\n');
    process.exit(1);
  }
