 ...
 *   node scan.mjs --user <username> --verify --headed-fallback  # retry anti-bot-blocked URLs in a headed browser
 *   node scan.mjs --user <username> --verify --throttle          # jittered ~5-10s gap between checks
 *   node scan.mjs --user <username> --verify --throttle=8000     # custom base gap in ms (waits base..2*base)
 *   node scan.mjs --user <username> --include-blacklisted         # let users/{USER}/data/blacklist.md matches through (annotated)
 */
...
let PROFILE_PATH = 'config/profile.yml';
...
let BLACKLIST_PATH = 'data/blacklist.md';
let SCAN_RUNS_PATH = 'data/scan-runs.tsv';
let PORTAL_HEALTH_PATH = 'data/portal-health.tsv';
...
function configureUserPaths(ctx) {
  ...
  PROFILE_PATH = process.env.CAREER_OPS_PROFILE
    ? path.resolve(process.env.CAREER_OPS_PROFILE)
    : userPath(ctx, 'config/profile.yml');
  // A second search lane needs both its own inbox and its own dedup history.
  // Keep the normal active-user defaults, while allowing explicit lane paths
  // to move those files without reintroducing root-level candidate data.
  SCAN_HISTORY_PATH = process.env.CAREER_OPS_SCAN_HISTORY
    ? path.resolve(process.env.CAREER_OPS_SCAN_HISTORY)
    : userPath(ctx, 'data/scan-history.tsv');
  PIPELINE_PATH = process.env.CAREER_OPS_PIPELINE
    ? path.resolve(process.env.CAREER_OPS_PIPELINE)
    : userPath(ctx, 'data/pipeline.md');
  APPLICATIONS_PATH = userPath(ctx, 'data/applications.md');
  SCAN_HANDOFF_PATH = userPath(ctx, 'data/scan-handoff.json');
  BLACKLIST_PATH = userPath(ctx, 'data/blacklist.md');
  SCAN_RUNS_PATH = userPath(ctx, 'data/scan-runs.tsv');
  PORTAL_HEALTH_PATH = userPath(ctx, 'data/portal-health.tsv');
}

export function configureScanUserPaths(ctx) {
  configureUserPaths(ctx);
}
...
// Fork compatibility: the scanner needs row buckets, not only a flat key set,
// so a newly live duplicate can reopen the original Closed row in place. Keys
// use upstream's alias-aware company and role normalization.
function loadSeenCompanyRoleBuckets(appsPath = APPLICATIONS_PATH, canonicalize = defaultCompanyNormalizer) {
  const seen = new Map();
  if (!existsSync(appsPath)) return seen;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!app) continue;
    const key = companyRoleDedupKey(app.company, app.role, canonicalize);
    if (!key) continue;
...
export function reopenClosedDuplicateApplications(offers, date, canonicalize = defaultCompanyNormalizer) {
...
async function main() {
  ...
  await mergeProviderPlugins(providers, { root: path.dirname(PROVIDERS_DIR), configRoot: userContext.userRoot });
  if (providers.size === 0) {
  ...
  const seenCompanyRoleBuckets = loadSeenCompanyRoleBuckets(APPLICATIONS_PATH, canonicalizeCompany);
  ...
  const tasks = targets.map(company => async () => {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        if (!targetLocationFilter(job.location, job.url, job.title)) {
        ...
        const seenCompanyRole = seenCompanyRoleBuckets.get(key);
        if (seenCompanyRoles.has(key)) {
          totalDupes++;
          seenUrls.add(dedupUrl);
          let reopenTarget = null;
          if (seenCompanyRole && seenCompanyRole.activeRows.length === 0 && seenCompanyRole.closedRows.length > 0) {
            reopenTarget = seenCompanyRole.closedRows[0];
          } else if (seenCompanyRole && seenCompanyRole.reopenableRows.length > 0) {
        ...
        seenCompanyRoleBuckets.set(key, { activeRows: [{ company: job.company, role: job.title, status: 'Evaluated' }], closedRows: [], reopenableRows: [] });
  ...
  if (!dryRun && reopenedClosedDuplicates.length > 0) {
    totalReopenedClosedDupes = reopenClosedDuplicateApplications(reopenedClosedDuplicates, date, canonicalizeCompany);
  ...
  if (persistentlyDead.length > 0) {
    ...
    console.log(`   Run: node verify-portals.mjs --file ${PORTALS_PATH} to check if the ATS migrated, or update their board slugs.`);
  }
  if (newlyDeadSlug.length > 0) {
    const names = newlyDeadSlug.map(x => x.company).join(', ');
    console.log(`\n⚠️  ${newlyDeadSlug.length} target(s) unreachable (slug?): ${names} — run: node verify-portals.mjs --file ${PORTALS_PATH}`);
  }
