...
function usage() {
  console.log(`Usage:
  node company-funded.mjs --user USER --dry-run
  node company-funded.mjs --user USER --limit 25 --months 6 --sort date
  node company-funded.mjs --user USER --sources techcrunch,prnewswire,guardian,hn --dry-run
...
function writeArtifacts(result, userRoot) {
  const outDir = join(userRoot, 'output');
  const reportDir = join(userRoot, 'reports');
...
async function main() {
  ...
  if (!opts.dryRun) result.artifacts = writeArtifacts(result, userContext.userRoot);
