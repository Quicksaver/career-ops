...
3. Once approved + pinned, anyone who runs `node plugins.mjs --user <username> add career-ops-plugin-<id>`
   installs your version, and the engine gives **your maintained successor
   precedence over the bundled reference** of the same id. `node plugins.mjs --user <username> available`
...
you edit `users/{USER}/plugins.local/` or your `users/{USER}/plugins.lock`, you're choosing to run your own
