...
const userLayerIndex = agentLines.findIndex(l => l.includes('**User Layer'));
const declaration = userLayerIndex >= 0
  ? agentLines.slice(userLayerIndex + 1).filter((line, index, all) => {
      const firstBlank = all.findIndex(candidate => candidate.trim() === '');
      return firstBlank === -1 || index < firstBlank;
    }).join('\n')
  : '';
...
if (userLayerIndex < 0) {
  ...
  const paths = [...declaration.matchAll(/`([^`]+)`/g)]
    .map(m => m[1])
