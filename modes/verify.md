...
- `restore_orphan` for a valid evaluation whose tracker row was lost. Restoration prefers a matching preserved TSV under `users/{USER}/batch/tracker-additions/merged/`; when none survives, a complete report can reconstruct the bounded tracker fields. If its numeric prefix is already occupied, the applier assigns a fresh ID and renames the report plus matching HTML/PDF/TeX artifacts together.
