...
function usage() {
  return `Usage: node verify-cv-facts.mjs --user <id> <generated-document> [--source path] [--config path] [--json]
       node verify-cv-facts.mjs --self-test
...
Default sources: users/<id>/cv.md, users/<id>/article-digest.md
Default config:  users/<id>/config/cv-facts.json (optional)`;
}
...
export function runCli(args = process.argv.slice(2)) {
  ...
  try {
    userContext = getUserContext(args, {
      requireUser: !args.includes('--help') && !args.includes('-h'),
    });
  } catch (err) {
  ...
  let parsed;
  try {
    parsed = parseCliArgs(userContext.args);
  } catch (err) {
  ...
  try {
    const defaultSources = userContext.userRoot
      ? [userPath(userContext, 'cv.md'), userPath(userContext, 'article-digest.md')]
      : DEFAULT_SOURCES;
    const configPath = parsed.configPath === DEFAULT_CONFIG && userContext.userRoot
      ? userPath(userContext, 'config/cv-facts.json')
      : parsed.configPath;
    const result = verifyFacts(readFileSync(targetPath, 'utf-8'), {
      sourcePaths: parsed.sourcePaths.length ? parsed.sourcePaths : defaultSources,
      configPath,
    });
    if (parsed.json) {
    ...
    console.error('\nAdd real evidence to the active user cv.md/article-digest.md, or allow a verified exception in config/cv-facts.json.');
    return 1;
  } catch (err) {
    if (parsed.json) {
