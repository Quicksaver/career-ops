...
async function cmdRun(args) {
  ...
  if (hook === 'ingest' || hook === 'search') {
    ...
    if (jobs.length) { await appendToPipeline(jobs); console.log(`→ Appended ${jobs.length} to users/${userContext.userId}/data/pipeline.md. Run /career-ops pipeline ${userContext.userId} to evaluate.`); }
    return;
  }
