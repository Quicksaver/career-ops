 ...
 *   6. On the default path, a first `add` writes to the active user's
 *      users/{USER}/data/agent-inbox.md path.
 *
...
// ---------------------------------------------------------------------------
console.log('6. default path is scoped to the active user');
{
  const repo = tmp('inbox-repo-');
  writeFileSync(join(repo, '.gitignore'), 'node_modules\noutput/*\n');
  const usersRoot = join(repo, 'users');
  const addOnce = () => execFileSync(NODE, [CLI, '--user', 'test', 'add', 'queue a scan'], {
    cwd: repo, env: { ...process.env, CAREER_OPS_USERS_DIR: usersRoot }, stdio: ['pipe', 'pipe', 'pipe'],
  });
  addOnce(); addOnce();
  const md = readFileSync(join(usersRoot, 'test', 'data', 'agent-inbox.md'), 'utf8');
  check('default inbox is written under users/test/data', (md.match(/^- \[ \]/gm) || []).length === 2, md);
}
