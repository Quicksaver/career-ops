...
Current tracker links and active report contents define present identity. Historical batch completion files are provenance rather than canonical state: they may expose a report-number collision, but a superseded reservation does not override an otherwise consistent current tracker/report pair. The raw verifier emits `duplicate_report_number` as a blocking error whenever two active report files share one numeric prefix; reviewers must leave that collision for human repair rather than guessing which file owns the ID.
