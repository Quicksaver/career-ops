...
> Apply `users/{USER}/voice-dna.md` (if present) to every generated message — full guardrail, conversational voice included (Tier 1 + Tier 2). See `_writing.md` → Voice DNA.
...
7. **Offer to save the contact** -- once the candidate picks a target, ask whether
   ...
   `node contacts.mjs --vcf --user {USER}` (vCard).
