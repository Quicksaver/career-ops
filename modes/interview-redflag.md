...
Requires transcripts produced by `modes/interview/debrief.md` or `modes/interview/practice.md` (#956). Each transcript must be saved under `users/{USER}/interview-prep/sessions/` (gitignored). If no session files exist, exit gracefully with an onboarding message.

## Inputs

- `users/{USER}/interview-prep/sessions/` — Session transcripts (debrief + practice outputs). One file per round.
- `users/{USER}/interview-prep/{company}-{role}.md` — Company intel file (for context + output target).
- `users/{USER}/config/profile.yml` — User profile (for role/archetype context and the candidate's location used for Step 2c jurisdiction derivation).
- `templates/protected-grounds.yml` — Jurisdiction-keyed protected-ground/do-not-ask topics used only by Step 2c. It is a local data reference, not instruction logic.
- **Original JD text (user-provided, for Step 2b only)** — the posted job description for the role under analysis. Same "user-provided input, not automated scraping" pattern used elsewhere in this codebase (e.g. `jd-skill-gap.mjs`): paste it, or point at `users/{USER}/jds/{file}` if it's already saved there. Without it, Step 2b is skipped — every other step runs as normal.
...
Check: does `users/{USER}/interview-prep/sessions/` contain at least **1** file?
...
List all `.md` files in `users/{USER}/interview-prep/sessions/`. Parse each filename using the `YYYY-MM-DD` date segment as the anchor:
...
[Render in {language.output}: a note that this is a suggestion only — nothing is written to `users/{USER}/data/blacklist.md` automatically.]}
...
→ Full analysis written to users/{USER}/interview-prep/{company-slug}-redflags.md
```
...
```text
→ [render in {language.output}, e.g. "Blacklist entry suggested — see"] users/{USER}/interview-prep/{company-slug}-redflags.md
```
...
- **No auto-action** — never edits `users/{USER}/portals.yml`, `users/{USER}/modes/_profile.md`, `users/{USER}/data/applications.md`, or `users/{USER}/data/blacklist.md`. At `🚩 Reconsider`, the output suggests a ready-to-copy blacklist row; the user (or their agent, on explicit instruction) still has to add it themselves. Advisory only.
