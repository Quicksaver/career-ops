...
try {
  const { buildCompanyFilter, buildLocationFilter, shouldDedupScanHistoryRow } = await import(pathToFileURL(join(ROOT, 'scan.mjs')).href);
...
try {
  ...
  const env = { ...process.env, PATH: `${fakeBin}:${process.env.PATH}`, CAREER_OPS_USERS_DIR: usersDir };
  const out = run('bash', [join(batchDir, 'batch-runner.sh'), '--user', 'test', '--parallel', '1', '--max-retries', '3', '--rate-limit-sleep', '0'], {
    cwd: tmp,
    env,
  ...
  const dry = run('bash', [join(batchDir, 'batch-runner.sh'), '--user', 'test', '--resume-paused', '--dry-run'], {
    cwd: tmp,
    env,
    stdio: ['pipe', 'pipe', 'pipe'],
  }) || '';
