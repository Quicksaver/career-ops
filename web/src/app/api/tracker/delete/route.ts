...
import { careerOpsCoreEnv, careerOpsSystemRoot, rootScript, trackerCanDelete } from "@/lib/career-ops";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await new Promise<{ code: number | null; err: string }>((resolve) => {
      ...
      let child;
      try {
        child = spawn(process.execPath, args, { cwd: careerOpsSystemRoot(), env: careerOpsCoreEnv() });
      } catch (e) {
