...
**6. Employment Classification Risk** (from JD text; jurisdiction from `users/{USER}/config/profile.yml` → `location.country`):
...
| Interview red flags | `users/{USER}/interview-prep/{company-slug}-redflags.md` (from `interview-redflag` mode) | **Cross-reference, not a copy:** if the file exists, surface its current warning level plus a relative link — `[{level}](../interview-prep/{company-slug}-redflags.md)` (relative to `users/{USER}/reports/`); otherwise `— no interview sessions yet` |
...
Never infer a desired number from the JD, the score, or past conversations. The profile default (`users/{USER}/config/profile.yml` → `compensation.target_range`) needs no line — `salary-gap.mjs` reads it as the fallback. The advertised figure also needs no line: the report's `advertised_comp` **is** the advertised observation.
