...
func NewViewerModel(t theme.Theme, path, title string, width, height int, app model.CareerApplication, careerOpsPath string) ViewerModel {
	return NewViewerModelWithFileRoot(t, path, title, careerOpsPath, width, height, app, careerOpsPath)
}
...
func NewViewerModelWithFileRoot(t theme.Theme, path, title, fileRoot string, width, height int, extras ...any) ViewerModel {
	var app model.CareerApplication
	var careerOpsPath string
	if len(extras) > 0 {
		if value, ok := extras[0].(model.CareerApplication); ok {
			app = value
		}
	}
	if len(extras) > 1 {
		if value, ok := extras[1].(string); ok {
			careerOpsPath = value
		}
	}
